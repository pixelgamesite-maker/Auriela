...
const WL_LS_KEY = "aurelia_wl_applied";

// ── Aurelia X links for WL tasks ───────────────────────────────────────────────
...
// ── Whitelist Modal ────────────────────────────────────────────────────────────
function WhitelistModal({ onClose, prefillHandle = "" }: { onClose: () => void; prefillHandle?: string }) {
  ...
  return (
    <div style={{
      background: "var(--panel)", borderRadius: 18, border: `1px solid var(--border)`,
      padding: "28px 24px", maxWidth: 480, width: "100%", maxHeight: "85vh", overflowY: "auto",
      position: "relative",
    }}>
      <button
        onClick={onClose}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {step2 && (
          <TaskCard delay={60}>
            ...
            {!followDone && (
              <button
                onClick={() => openAndMark(AURELIA_X, () => setFollowDone(true))}
                style={{
        ...
        {step3 && (
          <TaskCard delay={60}>
            ...
            {!likeQuoteDone ? (
              <>
                <button
                  onClick={() => openAndMark(AURELIA_POST, () => setLikeQuoteDone(true))}
                  style={{
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: done ? 0 : 14 }}>
      ...
      {done && (
        <span style={{
          ...
          fontWeight: 700, color: "var(--gold)", letterSpacing: "0.08em", textTransform: "uppercase",
          fontFamily: "system-ui",
        }}>
          <Sparkle size={8} /> Applied
        </span>
      )}
    </div>
  );
...
function GatedLanding({ onSignIn, onApplyWhitelist }: { onSignIn: () => void; onApplyWhitelist: () => void }) {
  ...
  if (!user) {
    return (
      <div style={{ fontFamily: "'Cormorant Garamond', 'Playfair Display', Georgia, serif" }}>
        ...
        <GatedLanding onSignIn={signInWithTwitter} onApplyWhitelist={() => setShowWlModal(true)} />
        {showWlModal && (
          <div
            style={{
